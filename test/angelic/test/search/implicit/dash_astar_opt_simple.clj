(ns angelic.test.search.implicit.dash-astar-opt-simple
  (:use clojure.test
        angelic.search.implicit.dash-astar-opt-simple)
...
(def ^:private all-variants
     [[:dash [:gather true :d true :s :eager :dir :right]]])
...
(deftest hierarchical-discrete-manipulation
  (doseq [[alg-name opts] all-variants]
    (let [alg #(apply implicit-dash-a*-opt % opts)]
     (testing alg-name
