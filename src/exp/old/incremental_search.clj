...
(defmethod queues/get-cost exp.old.incremental_search.Summary [x] (- (max-reward x)))
...
(deftype SimpleNode [name reward soln children data]
  Comparable (compareTo  [x] 
               (let [c  (- (max-reward x) reward)]
                 (if (not (zero? c)) c
                   (cond soln -1 
                         (and (instance? exp.old.incremental_search.Node x) (solution x)) 1
                         :else 0))))
  Summary (max-reward [] reward)   
  Node    (node-name  [] name)
          (solution   [] soln)
