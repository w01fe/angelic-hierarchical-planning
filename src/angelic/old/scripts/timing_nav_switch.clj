(ns angelic.old.scripts.timing-nav-switch
 (:require [angelic.util :as util] [angelic.old  [envs :as envs] [search :as search] [angelic :as angelic]] 
	   ...
	   [angelic.old.domains.nav-switch :as nav-switch]
	   [angelic.old.search.algorithms.textbook :as textbook]
	   [angelic.old.angelic [dnf-valuations :as dnf-valuations]
...
(defn- time-and-check-hierarchical [str reward hierarchy-schema env val-type]
  ...
  (let [initial-hla (hierarchies/instantiate-hierarchy hierarchy-schema env)
	node (angelic.old.angelic.algorithms.abstract-lookahead-trees/make-initial-alt-node initial-hla {:valuation-type val-type :cache? false :graph? false})]
  (util/assert-is 
