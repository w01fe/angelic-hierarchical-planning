...
(defn test-icaps-ww [i alt-args]
  (let [e (nth *icaps-ww* i)
    ...
    (doseq [[alg node]
	    [[textbook/a-star-graph-search (search/ss-node e (fn [s] (angelic/valuation-max-reward (angelic/progress-valuation (angelic/state->valuation :angelic.old.angelic.dnf-valuations/DNFValuation s) d))))]
...
; (binding [*debug-level* 2] (let [e (nth *icaps-ww* 0) d (ground-description (instantiate-description-schema (parse-description [:warehouse-act] nil nil) e) {})] (interactive-search (ss-node e (fn [s] (valuation-max-reward (progress-valuation (state->valuation :angelic.old.angelic.dnf-valuations/DNFValuation s) d)))) (make-graph-search-pq) #(- (upper-reward-bound %)))))
