(ns angelic.old.angelic.hierarchies.flat
...
(defmethod progress-valuation [:angelic.old.angelic/Valuation ::FlatActOptimisticDescription] [val desc]
...
(defmethod hla-default-optimistic-valuation-type ::FlatActHLA [hla] 
  :angelic.old.angelic/ExplicitValuation)
...
(defmethod hla-immediate-refinements [::FlatPrimitiveHLA :angelic.old.angelic/Valuation] [hla val] nil)
(defmethod hla-immediate-refinements [::FlatActHLA :angelic.old.angelic/Valuation]       [hla val]
  (let [state-map (explicit-valuation-map val)]
    (util/assert-is (= (count state-map) 1))
    (let [[prev-state prev-reward] (first state-map)]
      (cons [] 
