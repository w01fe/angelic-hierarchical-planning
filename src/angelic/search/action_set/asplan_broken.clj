(ns angelic.search.action-set.asplan-broken
...
;; NOTE: this version is broken, see, e.g., domains.pathological/top-down
         ...
         (util/cond-let [x]
          ...
          (util/find-first #(not (state/get-var s (free-var %))) (map second na-tuples))  
            (do (println "A!" x );                         (state/get-var s x) (state/get-var s (parent-var x sas/goal-var-name)))
             (for [as (vals (get-in dtgs [x (state/get-var s x)])), a as]
               (make-add-action-action a)))
...
;; (use 'angelic.env 'angelic.hierarchy 'angelic.search.textbook 'angelic.domains.taxi-infinite 'angelic.search.action-set.asplan  'angelic.domains.sas-problems 'angelic.util 'angelic.sas.hm-heuristic)
