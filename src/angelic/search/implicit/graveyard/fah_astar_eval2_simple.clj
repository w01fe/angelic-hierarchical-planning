(ns angelic.search.implicit.fah-astar-eval2-simple
...
;; This ons is before refine-input/subsumption change.
...
(defn can-evaluate? [s]  (instance? angelic.search.implicit.fah_astar_eval2_simple.Evaluable s))
...
(defmethod print-method angelic.search.implicit.fah_astar_eval2_simple.Subproblem [ss o] (print-method (sp-str ss) o))
...
(defmethod print-method angelic.search.implicit.fah_astar_eval2_simple.TreeSummarizer [ss o] (print-method (ts-str ss) o))
...
(defn make-simple-subproblem [name inp-set out-set summary-fn inner-children copy-children child-transform]
  (let [ts-atom (atom nil)
    ...
    (doseq [child copy-children]
      (add-watcher! child (fn [_ out-sp] (connect-child-sp! ts (child-transform out-sp)))))
