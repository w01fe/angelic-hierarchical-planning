(ns angelic.search.implicit.dash-astar-opt-old
...
(defmethod print-method angelic.search.implicit.dash_astar_opt_old.Stub [s o]
...
(defn- can-evaluate? [s] (instance? angelic.search.implicit.dash_astar_opt_old.Evaluable s))
...
(defn- connect-and-watch-stub! [p c f]
  (assert (instance? angelic.search.implicit.dash_astar_opt_old.Stub c))
...
(defmethod print-method angelic.search.implicit.dash_astar_opt_old.Subproblem [sp o]
...
(defn- tree-summarizer? [s] (instance? angelic.search.implicit.dash_astar_opt_old.TreeSummarizer s))
...
(defn- ts-str [sp])
(defmethod print-method angelic.search.implicit.dash_astar_opt_old.TreeSummarizer [ts o]
...
;; (do (use '[angelic env hierarchy] 'angelic.domains.nav-switch 'angelic.search.implicit.fah-astar-expand 'angelic.search.implicit.fah-astar-eval 'angelic.search.implicit.dash-astar-opt-old 'angelic.domains.discrete-manipulation) (require '[angelic.search.explicit.hierarchical :as his]))

;; (do (use '[angelic env hierarchy] 'angelic.domains.nav-switch  'angelic.search.implicit.dash-astar-opt-old 'angelic.domains.discrete-manipulation) (require '[angelic.search.explicit.hierarchical :as his]))
...
;; Compare this and old version with same opts.
;; (dotimes [_ 1] (reset! summaries/*summary-count* 0) (reset! summaries-old/*summary-count* 0) (reset! da/*out-count* 0) (reset! dao/*out-count* 0) (debug 0 (let [opts [:gather false :d false :s nil] h (make-nav-switch-hierarchy (make-random-nav-switch-env 5 3 0) true)]  (time (println (run-counted #(identity (apply da/implicit-dash-a* h opts))) @summaries/*summary-count* @da/*out-count*))  (time (println (run-counted #(identity (apply dao/implicit-dash-a*-opt h opts))) @summaries-old/*summary-count*  @dao/*out-count*)) )))
